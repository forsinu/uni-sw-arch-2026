...
from pydantic import AnyUrl, HttpUrl, PostgresDsn, computed_field
...
class EnvHandler(BaseSettings):
    ...
    LIVE_RESULT_HOST: str = "live-result-service"
    LIVE_RESULT_PORT: int = 8000

    API_PREFIX: str
    API_VERSION: int = 1

    ROUTING_LOGGER_NAME: str = "competition-service.routing"
    ...
    DB_LOGGER_NAME: str = "competition-service.database"
    ...
    APP_LOGGER_NAME: str = "competition-service"
    LOGGER_HANDLER_NAME: str = "competition-service.logger"
    ...
    @computed_field
    @property
    def LIVE_RESULT_URL(self) -> str:
        return f"http://{self.LIVE_RESULT_HOST}:{self.LIVE_RESULT_PORT}"
