...
export class CoachComponent {
  ...
  protected readonly selectedFederationId = signal('');
  protected readonly selectedAthlete = computed(() => this.athletes().find((athlete) => athlete.federationId === this.selectedFederationId()) ?? null);
  ...
  constructor() { this.reload(); this.form.controls.meetingId.valueChanges.subscribe((meetingId) => this.loadEvents(meetingId)); this.form.controls.federationId.valueChanges.subscribe((federationId) => this.selectedFederationId.set(federationId)); }
  protected isCoach(): boolean { return this.auth.federationContext().federationRole === 'COA'; }
  protected athleteLabel(athlete: FederationMember): string { return `${athlete.firstName} ${athlete.lastName} (${athlete.memberCode})`; }
  protected selectAthlete(athlete: FederationMember): void { this.form.patchValue({ federationId: athlete.federationId }); }
